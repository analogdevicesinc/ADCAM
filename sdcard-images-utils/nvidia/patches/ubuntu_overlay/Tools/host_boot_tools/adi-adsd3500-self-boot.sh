...
run_self_boot_sequence() {
    adsd3500_power_sequence
    echo "Self boot completed"
}

run_self_boot_sequence_config3() {
    # TODO: self boot sequence for ADSD3500/ADTF3066 + RGB is not yet defined.
    echo "Self boot sequence for CONFIG 3 (ADSD3500/ADTF3066 + RGB) is not yet implemented. Skipping."
}
...
main() {
    ...
    case "$CONFIG" in
        0)
            echo "Skipping self boot: no matching module badge combination found"
            exit 0
            ;;
        1|2)
