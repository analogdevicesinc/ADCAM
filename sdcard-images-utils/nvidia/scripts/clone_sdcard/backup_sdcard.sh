...
find_device() {
    for dev in /sys/block/mmcblk*; do
        devname=$(basename "$dev") # e.g., mmcblk0
        # Correct partition count by looking for partition names
        part_count=$(ls /sys/block/$devname | grep -c "^${devname}p[0-9]*$")

        if [ "$part_count" -eq 15 ]; then
            model=$(lsblk -dn -o MODEL "/dev/$devname" | xargs)
            echo "Device /dev/$devname has 15 partitions. Model: $model"

            base_device_symlink=$(ls -1 /dev/disk/by-id/ | grep -i "$model" | grep -Ev -- '-part[0-9]+$' | head -n1)

            if [ -n "$base_device_symlink" ]; then
                SRC="/dev/disk/by-id/$base_device_symlink"
...
perform_backup() {
    ...
    OUT="$(pwd)/jetson_orin_nano_sdcard_$(date +%Y%m%d_%H%M).img.zst"
