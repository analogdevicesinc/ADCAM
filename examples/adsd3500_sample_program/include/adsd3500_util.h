...
// Use ADCAM's ToFi compute headers instead of local depthComputeLibrary
#include "adi/tofi/tofi_compute.h"
#include "adi/tofi/tofi_config.h"
...
// NOTE: IDs updated to match nv_adsd3500 driver on Jetson
#define V4L2_CID_AD_DEV_CHIP_CONFIG (0x9819d1) // chip_config (was 0x9819e1)
#define CTRL_SET_MODE (0x9819d0)               // operating_mode (was 0x9819e0)
#define CTRL_AB_AVG (0x9819d5)                 // ab_averaging (was 0x9819e5)
#define CTRL_DEPTH_EN (0x9819d6)               // depth_enable (was 0x9819e6)
#define CTRL_PHASE_DEPTH_BITS (0x9819d2)       // phase_depth_bits (same)
...
// NOTE: Adjusted for NVIDIA Jetson - ADSD3500 is at subdev2, not subdev1
#define CAMERA_SENSOR_DRIVER "/dev/v4l-subdev2"
#define VIDEO_CAPTURE_DRIVER "/dev/video0"
// NOTE: Jetson uses tegra-video, not mxc-isi-cap - match on "vi-output, adsd3500"
#define CAPTURE_DEVICE_NAME "vi-output, adsd3500"
...
struct Metadata {
     ...
     * In partial depth case, the host must know the invalid phase value used
     * by the ADSD3500, which is used for invalidation during full depth
     * compute.
     */
    uint16_t invalidPhaseValue;

    /**
     * @brief frequencyIndex: Stores index of the frequency for which the
     * phase frame is outputted.
     */
    ...
    /**
     * @brief True if xyz is being generated for the current frame. (set by
     * sdk)
     */
    uint8_t xyzEnabled;
...
// CCB Versions (updated to match SDK support)
