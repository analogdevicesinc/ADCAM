...
int Adsd3500InterruptNotifier::enableInterrupts() {
    ...
    // NOTE: Updated to match ADCAM SDK - uses /proc not /sys/kernel/debug
    const char *proc_name = "/proc/adsd3500/value";
    int debug_fd = ::open(proc_name, O_RDWR);
    if (debug_fd < 0) {
        std::cout << "Failed to open " << proc_name << ". "
                  << "Interrupts support will not be available!" << std::endl;
        return -1;
    }
