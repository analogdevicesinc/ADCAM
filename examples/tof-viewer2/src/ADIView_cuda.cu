...
__global__ void minMaxKernel(uint16_t* buffer, int size, uint32_t* minVal, uint32_t* maxVal) {
...
__global__ void normalizeKernel(uint16_t* buffer, int width, int height, 
                                uint32_t minVal, uint32_t maxVal, float normFactor) {
...
__global__ void convertToBGRKernel(uint16_t* input, uint8_t* output, int width, int height) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void processDepthKernel(uint16_t* depth, uint8_t* bgr, 
...
__global__ void processPointCloudKernel(int16_t* xyz, float* vertices, uint8_t* abData,
                                        int width, int height, float maxX, float maxY, float maxZ,
                                        int minRange, int maxRange, int pcColour, bool haveAb) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (x < width && y < height) {
        ...
        if (xyz[xyzIdx + 2] == 0) {
            ...
            if (pcColour == 2) {
                ...
                int clampedVal = min(max((int)xyz[xyzIdx + 2], minRange), maxRange);
                float hue = (clampedVal - minRange) / (float)(maxRange - minRange);
                hue *= (2.0f / 3.0f);
...
void normalizeABBuffer_CUDA(uint16_t* d_abBuffer, uint16_t* h_abBuffer,
    ...
    // Copy to device
    CUDA_CHECK(cudaMemcpy(d_buffer, h_abBuffer, imageSize * sizeof(uint16_t), cudaMemcpyHostToDevice));
    ...
    if (advanceScaling) {
        ...
        uint32_t initMax = 0;
        CUDA_CHECK(cudaMemcpy(d_min, &initMin, sizeof(uint32_t), cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_max, &initMax, sizeof(uint32_t), cudaMemcpyHostToDevice));
        
        // Launch min/max kernel
        int blockSize = 256;
        int numBlocks = (imageSize + blockSize - 1) / blockSize;
        minMaxKernel<<<numBlocks, blockSize, blockSize * 2 * sizeof(uint32_t)>>>(d_buffer, imageSize, d_min, d_max);
        
        CUDA_CHECK(cudaMemcpy(&minVal, d_min, sizeof(uint32_t), cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaMemcpy(&maxVal, d_max, sizeof(uint32_t), cudaMemcpyDeviceToHost));
    ...
    normalizeKernel<<<gridDim, blockDim>>>(d_buffer, abWidth, abHeight, minVal, maxVal, normFactor);
    ...
    if (useLogScaling) {
        ...
        logScaleKernel<<<gridDim, blockDim>>>(d_buffer, abWidth, abHeight, minVal, maxLogVal);
    }
    
    // Copy back to host
    CUDA_CHECK(cudaMemcpy(h_abBuffer, d_buffer, imageSize * sizeof(uint16_t), cudaMemcpyDeviceToHost));
}
...
void convertABtoBGR_CUDA(uint16_t* d_abBuffer, uint8_t* d_bgrBuffer,
    ...
    CUDA_CHECK(cudaMemcpy(d_input, d_abBuffer, imageSize * sizeof(uint16_t), cudaMemcpyHostToDevice));
    ...
    CUDA_CHECK(cudaMemcpy(h_bgrBuffer, d_output, imageSize * 3 * sizeof(uint8_t), cudaMemcpyDeviceToHost));
}
...
void processDepthImage_CUDA(uint16_t* d_depthBuffer, uint16_t* h_depthBuffer,
    ...
    if (d_depth == nullptr || lastSize != imageSize) {
        if (d_depth) CUDA_CHECK(cudaFree(d_depth));
        if (d_bgr) CUDA_CHECK(cudaFree(d_bgr));
    ...
    CUDA_CHECK(cudaMemcpy(d_depth, h_depthBuffer, imageSize * sizeof(uint16_t), cudaMemcpyHostToDevice));
    
    dim3 blockDim(16, 16);
    dim3 gridDim((width + blockDim.x - 1) / blockDim.x,
                 (height + blockDim.y - 1) / blockDim.y);
    
    processDepthKernel<<<gridDim, blockDim>>>(d_depth, d_bgr, width, height, minRange, maxRange);
    
    CUDA_CHECK(cudaMemcpy(h_bgrBuffer, d_bgr, imageSize * 3 * sizeof(uint8_t), cudaMemcpyDeviceToHost));
}
...
void processPointCloud_CUDA(int16_t* d_xyzBuffer, int16_t* h_xyzBuffer,
    ...
    if (d_xyz == nullptr || lastSize != imageSize) {
        if (d_xyz) CUDA_CHECK(cudaFree(d_xyz));
        if (d_vertices) CUDA_CHECK(cudaFree(d_vertices));
        if (d_ab) CUDA_CHECK(cudaFree(d_ab));
        
        CUDA_CHECK(cudaMalloc(&d_xyz, imageSize * 3 * sizeof(int16_t)));
        CUDA_CHECK(cudaMalloc(&d_vertices, imageSize * 6 * sizeof(float))); // XYZ + RGB
        CUDA_CHECK(cudaMalloc(&d_ab, imageSize * 3 * sizeof(uint8_t)));
        lastSize = imageSize;
    }
    
    CUDA_CHECK(cudaMemcpy(d_xyz, h_xyzBuffer, imageSize * 3 * sizeof(int16_t), cudaMemcpyHostToDevice));
    
    if (haveAb && d_abBuffer != nullptr) {
        CUDA_CHECK(cudaMemcpy(d_ab, d_abBuffer, imageSize * 3 * sizeof(uint8_t), cudaMemcpyHostToDevice));
    }
    ...
    processPointCloudKernel<<<gridDim, blockDim>>>(d_xyz, d_vertices, d_ab, 
                                                    width, height, maxX, maxY, maxZ,
                                                    minRange, maxRange, pcColour, haveAb);
    
    CUDA_CHECK(cudaMemcpy(h_vertexBuffer, d_vertices, imageSize * 6 * sizeof(float), cudaMemcpyDeviceToHost));
}
