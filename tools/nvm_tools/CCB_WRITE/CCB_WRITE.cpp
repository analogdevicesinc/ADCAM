...
int main(int argc, char **argv) {
    ...
    printf("CCB Flash update app version: %d.%d.%d\n", VER_MAJ, VER_MIN,
           VER_PATCH);
    ...
    if (sizeOfBinary < MIN_BIN_SIZE || sizeOfBinary > MAX_BIN_SIZE) {
        ...
        std::cout << "CCB update failed " << std::endl;
        exit(0);
    }
    ...
    crc_output_t res = compute_crc(&crc_params, binbuff, sizeOfBinary);
    ...
    usleep(110 * 1000);
    if (retval == false) {
        std::cout << "ERROR! Could not send SWITCH_TO_BURST command!"
                  << std::endl;
        return -1;
    }
    memset(data, 0, 6);

    // Add 4 bytes crc in the chunk size
    sizeOfBinary += 4;
    ...
    data[5] = PAGE_SIZE >> 8;
    data[6] = WRITE_CCB_COMMAND;
    data[7] = sizeOfBinary & 0xFF;
    data[8] = sizeOfBinary >> 8;
    data[9] = sizeOfBinary >> 16;
    data[10] = sizeOfBinary >> 24;
    uint32_t checksum = 0;
    for (int i = 0; i < 7; i++) {
        checksum += data[i + 4];
    }
    ...
    if (Status_Command != ADI_STATUS_CCB_FLASH_UPDATE) {
        std::cout << "CCB write failed" << std::endl;
    } else {
        std::cout << "CCB Flash write completed and is successful."
                  << std::endl;
    }
